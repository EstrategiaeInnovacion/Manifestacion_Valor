...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Verificar Código | E&I</title>
                ...
                <div class="bg-white rounded-[3.5rem] shadow-2xl overflow-hidden border border-slate-100 p-10 lg:p-14">
                    <div class="text-center mb-10">
                        ...
                        <h2 class="text-4xl font-black text-[#001a4d] tracking-tight leading-tight">
                            Verifica tu <br><span class="text-[#003399]">Código.</span>
                        </h2>
                        <p class="text-sm text-slate-400 mt-3 leading-relaxed">
                            Enviamos un código de 6 dígitos a<br>
                            <strong class="text-[#001a4d]">{{ $maskedEmail }}</strong>
                        </p>
                    </div>

                    @if (session('recovery_sent'))
                        <div class="mb-5 p-4 rounded-2xl bg-green-50 border border-green-200 text-green-700 text-sm font-medium flex items-start gap-3">
                            <i data-lucide="check-circle" class="w-5 h-5 mt-0.5 flex-shrink-0"></i>
                            <span>Código enviado. Revisa tu correo — puede tardar unos momentos.</span>
                        </div>
                    @endif
                    ...
                    @if ($expired)
                        <div class="mb-5 p-4 rounded-2xl bg-amber-50 border border-amber-200 text-amber-700 text-sm font-medium flex items-start gap-3">
                            <i data-lucide="clock" class="w-5 h-5 mt-0.5 flex-shrink-0"></i>
                            <span>El código ha <strong>expirado</strong>. Solicita uno nuevo con el botón de abajo.</span>
                        </div>
                    @else
                        <form method="POST" action="{{ route('password.verify.store') }}" class="space-y-5">
                            @csrf
                            <div>
                                <label for="code" class="block text-sm font-bold text-[#001a4d] mb-2 px-1">
                                ...
                                <input id="code"
                                       ...
                                       class="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-[#003399] focus:border-transparent transition-all outline-none placeholder:text-slate-300 text-center text-3xl font-black tracking-[0.6em]"
                                       placeholder="000000">
                    ...
                    <div class="mt-4">
                        <form method="POST" action="{{ route('password.resend') }}">
                            @csrf
                            <button type="submit"
                                    class="w-full bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-600 font-bold py-4 px-8 rounded-2xl transition flex items-center justify-center gap-3">
                    ...
                    <div class="mt-5 text-center">
                        <a href="{{ route('password.request') }}"
                           class="text-sm font-bold text-[#003399] hover:underline inline-flex items-center gap-2">
