...
(async () => {
    console.log('Iniciando compilación de PDF del Manual de Programador...');
    const markdownPath = path.join(__dirname, '../docs/programmer/manual-programador.md');
    const pdfOutputPath = path.join(__dirname, '../docs/programmer/Manual de Programador.pdf');
    ...
    if (!fs.existsSync(markdownPath)) {
        console.error('No se encontró el manual de programador en Markdown:', markdownPath);
        return;
    }
    ...
    const baseHref = `file:///${path.join(__dirname, '../docs/programmer/').replace(/\\/g, '/')}`;

    // Plantilla HTML Completa con Estilos CSS Premium (Optimizados para código y diagramas)
    const fullHtml = `
<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <base href="${baseHref}">
    <title>Manual de Programador — Sistema de Manifestación de Valor Exterior (MVE)</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    
    <!-- Fuentes premium -->
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&family=Outfit:wght@600;700;800&family=Fira+Code:wght@400;500;600&display=swap" rel="stylesheet">
    
    <!-- Estilo de sintaxis para código (highlight.js) - GitHub Light -->
...
<body>
    ...
    <div class="cover">
        ...
        <div class="cover-title">Manual de Programador</div>
    ...
    <!-- Carga de highlight.js para resaltado de código -->
    ...
    // Guardar el HTML generado temporalmente
    const tempHtmlPath = path.join(__dirname, 'temp_programmer.html');
    ...
    await page.pdf({
        ...
        headerTemplate: '<div style="font-size: 8px; font-family: sans-serif; width: 100%; text-align: right; padding-right: 20px; color: #94a3b8;">Manual de Programador MVE</div>',
